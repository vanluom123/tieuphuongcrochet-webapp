'use client'
import { Input, Flex, Col, Pagination, Empty, Row, Spin, Affix } from 'antd'
    ...
    <div className="data-list">
      <Affix offsetTop={0} className="affix-search-area">
        <div>
          ...
          {/* Tabs categories */}
          {isShowTabs && items && (
            <CategoryMenu items={items as TabsItem[]} onClickMenu={(key) => onClickMenu(key)} />
          )}
        </div>
      </Affix>
            ...
            {dataSource &&
              dataSource.map((item, index) => (
                <div className="list-view-item" key={`list-view-item-${index}`}>
                  <ListViewItem data={item} onReadDetail={() => onReadDetail(item.key)} />
                </div>
              ))}
          </Flex>
        ) : (
