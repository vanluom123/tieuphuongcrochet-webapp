...
export const getCurrentDate = (d: Date | number | string, formatDate = 'DD/MM/YYYY') =>
  moment(d).format(formatDate)
...
export const uploadMultipleImagesToServer = async (
  ...
  const deletedImages =
    prevImages.filter(
      (item) => currentImages.findIndex((img) => img.fileName === item.fileName) < 0,
    ) || []

  // Delete images that are removed
  if (deletedImages?.length > 0) {
    await deleteMultipleFilesToR2(deletedImages.map((img) => img.fileName || ''))
  }
